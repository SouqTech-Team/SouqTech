            ...
            steps {
                ...
                script {
                    if (isUnix()) {
                        ...
                        echo '[INFO] Déploiement du Monitoring (Prometheus + Grafana + Alertmanager)...'
                        bat 'kubectl apply -f k8s/monitoring/prometheus-rbac.yaml'
                        bat 'kubectl apply -f k8s/monitoring/prometheus-rules.yaml'
                        ...
                        bat 'kubectl apply -f k8s/monitoring/alertmanager-config.yaml'
                        bat 'kubectl apply -f k8s/monitoring/alertmanager-deployment.yaml'
                        bat 'kubectl apply -f k8s/monitoring/alertmanager-service.yaml'
